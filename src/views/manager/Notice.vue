    ...
    <div class="table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="video" label="视频">
          <template v-slot="scope">
            <video v-if="scope.row.video" :src="scope.row.video" controls style="width: 100px; height: auto;"></video>
          </template>
        </el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form label-width="100px" style="padding-right: 50px" :model="form" :rules="rules" ref="formRef">
        ...
        <el-form-item v-if="form.shareType === 1" label="图片上传" prop="image">
          <el-upload :action="$baseUrl + '/notice/upload'" :headers="{ token: user.token }" list-type="picture"
            :before-upload="handleBeforeUpload" :on-success="handleImageSuccess" :on-error="handleUploadError">
            <el-button type="primary">上传图片</el-button>
          </el-upload>
        </el-form-item>
        <el-form-item v-if="form.shareType === 3" label="视频上传" prop="video">
          <el-upload :action="$baseUrl + '/notice/upload'" :headers="{ token: user.token }" list-type="video"
            :before-upload="handleBeforeUpload" :on-success="handleVideoSuccess" :on-error="handleUploadError">
...
export default {
  ...
  methods: {
    handleAdd() {   // 新增数据
      this.form = { shareType: 1 };  // 新增数据的时候清空数据并默认选择 "图片+文字"
