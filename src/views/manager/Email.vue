    ...
    <el-form label-width="100px" style="padding-right: 50px; margin: 20px;" :model="emailForm" :rules="emailRules"
      ...
      <el-form-item label="接收者类型" prop="receiverType">
        <el-radio-group v-model="emailForm.receiverType">
          ...
          <el-radio label="multiple">多个用户</el-radio>
      ...
      <el-form-item v-if="emailForm.receiverType === 'multiple'" label="接收者" prop="receiver">
        <el-select v-model="emailForm.receiver" placeholder="请选择接收者" multiple>
          <el-option v-for="item in receiverList" :key="item" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      emailRules: {
        ...
        receiver: [
          { required: true, message: '请选择接收者', trigger: 'change', type: 'array', min: 1 }
        ],
        subject: [
  ...
  methods: {
    fetchReceivers() {
      axios.get(`${process.env.VUE_APP_BASEURL}/email/receivers`).then(response => {
        this.receiverList = response.data;
      }).catch(error => {
        console.error('Error fetching receivers:', error);
        this.$message.error('获取接收者列表失败');
      });
    },
    sendEmail() {
      this.$refs.emailFormRef.validate((valid) => {
        if (valid) {
          let receiver;
          if (this.emailForm.receiverType === 'single') {
            receiver = this.emailForm.receiver;
          } else if (this.emailForm.receiverType === 'multiple') {
            receiver = this.emailForm.receiver.join(',');
          } else {
            receiver = '';
          }
